...
bottomPad = 100;%px
rightPad = 10;%px
% total for each control set
controlWidth = 400;%px
controlHeight = 140;%px
% simply knob positions
knobPosition = @(y) [65,  y+40,  60,  60];
lampPosition = @(y) [165, y+20,  26,  26];
editPosition = @(y) [235, y+45, 135,  50];
% compute required space
nControls = 12;
totalHeight = nControls*controlHeight + topPad + bottomPad;
totalWidth = controlWidth + rightPad;
%% Create Figure
fig = uifigure('Name','A Scrollable Panel!');
fig.Position(3) = 820;
%make a static viewport
viewHeight = fig.Position(4)-20;
viewPort= uipanel(fig,'position', [400,10,totalWidth,viewHeight]);
%Create a panel to conain the elements (this will be scrollable)
scrollPane= uipanel(viewPort);
scrollPane.Position = [1,1,totalWidth,totalHeight];
scrollPane.BackgroundColor = [1,1,1];
scrollPane.BorderType = 'none';
...
%initialze graphics placeholders
elementHandles = gobjects(nControls,3);
%find the starting y pixel entry
% I want to build the controls from top to bottom
startingY = totalHeight - topPad - controlHeight;
for j = 1:nControls
  %find each y-pixel control entry
  currentY = startingY - (j-1)*controlHeight;
  %create knob
  elementHandles(j,1) = uiknob(scrollPane, ... %scrollPane
    'position', knobPosition(currentY), ...
    'limits', [0,100], ...
    'value', 1 ...
    );
  %create lamp
  elementHandles(j,2) = uilamp(scrollPane, ... %scrollPane
    'position', lampPosition(currentY), ...
    'Color', [0,0,0] ...
    );
  %create editfield
  elementHandles(j,3) = uieditfield(scrollPane, ... %scrollPane
    'text', ...
    'position', editPosition(currentY), ...
    'editable', 'off', ...
